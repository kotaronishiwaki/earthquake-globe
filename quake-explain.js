...
- Be factual and specific to this region's tectonics. Do NOT restate the magnitude or place name in the text fields. Professional tone.`;
  ...
  let BACKEND = null; // null = unknown, true/false after first probe
  async function generate(q, lang) {
    ...
    if (BACKEND !== false) {
      try {
        ...
        if (r.status === 402) { BACKEND = true; const e = new Error('payment_required'); e.paywall = true; throw e; }
        if (r.ok) { BACKEND = true; return await r.json(); }
        // 404 / network → backend not present (preview); fall through.
        BACKEND = false;
      } catch (e) {
    ...
    // Preview fallback: use the sandbox-provided Claude.
