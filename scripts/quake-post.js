...
async function postToBluesky(text, lang, img, alt) {
  ...
  if (img) {
    try {
      ...
      embed = {
        ...
        images: [{ alt: alt || '', image: blob, aspectRatio: { width: 1200, height: 630 } }],
      };
    } catch (err) {
      console.error('  (Bluesky image upload skipped:', err.message + ')');
    }
  }
...
async function postToMastodon(text, lang, img, alt) {
  ...
  if (img) {
    try {
      ...
      fd.append('file', new Blob([img], { type: 'image/jpeg' }), 'quake.jpg');
      if (alt) fd.append('description', alt);
      const mRes = await fetch(`${root}/api/v2/media`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: fd,
      });
      if (!mRes.ok) throw new Error(`${mRes.status} ${await mRes.text()}`);
      mediaId = (await mRes.json()).id;
    } catch (err) {
      console.error('  (Mastodon image upload skipped:', err.message + ')');
    }
  }

  const payload = { status: text, language: lang, visibility: 'public' };
...
async function main() {
  ...
  for (const f of candidates) {
    ...
    // シェア画像（地球儀 + 震源）を生成。失敗してもテキスト投稿は続行。
    const img = await renderShareCard(q);
    const alt = `Globe map centered on the epicenter of a M${q.mag.toFixed(1)} earthquake — ${q.place}.` +
      (q.tsunami ? ' Tsunami may be possible.' : '');
    ...
    let ok = false;
    try {
      if (await postToBluesky(text, lang, img, alt)) { console.log('✅ Bluesky: posted'); ok = true; }
    } catch (err) {
      console.error('❌ Bluesky failed:', err.message);
    }
    try {
      if (await postToMastodon(text, lang, img, alt)) { console.log('✅ Mastodon: posted'); ok = true; }
    } catch (err) {
