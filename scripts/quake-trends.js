...
async function count(startMs, endMs) {
  // Use the standard query method and read metadata.count (the count method 404s).
  ...
  if (!Number.isFinite(n)) throw new Error(`Unexpected response for ${url}: ${JSON.stringify(j).slice(0, 200)}`);
  return n;
}
