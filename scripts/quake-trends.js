...
async function count(startMs, endMs) {
  const url = `${API}?format=geojson&minmagnitude=${MINMAG}&starttime=${iso(startMs)}&endtime=${iso(endMs)}`;
  const res = await fetch(url);
  if (res.status === 204) return 0;               // fdsnws uses 204 for "no events"
  if (!res.ok) throw new Error(`USGS responded ${res.status} for ${url}`);
  const j = await res.json();
  const n = Number(j && j.count);
  if (!Number.isFinite(n)) throw new Error(`Unexpected count response for ${url}: ${JSON.stringify(j)}`);
  return n;
}
